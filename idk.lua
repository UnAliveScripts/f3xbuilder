...
exportF3XBtn.MouseButton1Click:Connect(function()
	...
	local function buildF3X(filename, startIdx, endIdx, totalParts, canStream, chunkNum, totalChunks)
		...
		for i = startIdx, endIdx do
			...
			local fileMeshInfo = nil
			if includeMeshes then
				for _, child in ipairs(part:GetChildren()) do
					if child:IsA("SpecialMesh") then
						local mt = "Brick"
						pcall(function() mt = tostring(child.MeshType):gsub("Enum%.MeshType%.", "") end)
						if mt == "FileMesh" then
							local mid = ""; local tid = ""
							pcall(function() if child.MeshId and child.MeshId ~= "" then mid = child.MeshId end end)
							pcall(function() if child.TextureId and child.TextureId ~= "" then tid = child.TextureId end end)
							fileMeshInfo = {MeshId = mid, TextureID = tid}; break
						end
					end
				end
			end
			...
			local partOk, partJson = pcall(function()
				...
				pp(tostring(i))
				pp(',"ClassName":"')
				if fileMeshInfo then pp("MeshPart") else pp(part.ClassName) end
				pp('","Name":"')
				pp(part.Name:gsub('"', '\\"'))
				pp('"')
				...
				if fileMeshInfo then
					if fileMeshInfo.MeshId ~= "" then pp(',"MeshId":"'); pp(fileMeshInfo.MeshId); pp('"') end
					if fileMeshInfo.TextureID ~= "" then pp(',"TextureID":"'); pp(fileMeshInfo.TextureID); pp('"') end
				end
				...
				if part:IsA("MeshPart") then
					...
					local rf, cfv
					pcall(function() rf = tostring(part.RenderFidelity) end)
					pcall(function() cfv = tostring(part.CollisionFidelity) end)
					if rf then pp(',"RenderFidelity":"'); pp(rf); pp('"') end
					if cfv then pp(',"CollisionFidelity":"'); pp(cfv); pp('"') end
				end
					...
					for _, child in ipairs(part:GetChildren()) do
						if child:IsA("Decal") or child:IsA("Texture") then
							...
							if child:IsA("Texture") then
								local stu, stv = pcall(function() return child.StudsPerTileU end)
								if stu then cbuf[#cbuf + 1] = ',"StudsPerTileU":' .. string.format("%.4f", child.StudsPerTileU) end
								local stv, stvv = pcall(function() return child.StudsPerTileV end)
								if stv then cbuf[#cbuf + 1] = ',"StudsPerTileV":' .. string.format("%.4f", child.StudsPerTileV) end
							end
							cbuf[#cbuf + 1] = '}'
							childCount += 1
				...
				if includeMeshes then
					for _, child in ipairs(part:GetChildren()) do
						if child:IsA("SpecialMesh") then
							local meshType = "Brick"
							pcall(function() meshType = tostring(child.MeshType):gsub("Enum%.MeshType%.", "") end)
							if meshType ~= "FileMesh" then
								local cbuf = {}; local ci = 0
								local function cp(s) ci = ci + 1; cbuf[ci] = s end
								local meshId = ""; local texId = ""
								local scX, scY, scZ = 1, 1, 1; local ofX, ofY, ofZ = 0, 0, 0
								pcall(function() if child.MeshId and child.MeshId ~= "" then meshId = child.MeshId end end)
								pcall(function() if child.TextureId and child.TextureId ~= "" then texId = child.TextureId end end)
								pcall(function() scX, scY, scZ = child.Scale.X, child.Scale.Y, child.Scale.Z end)
								pcall(function() ofX, ofY, ofZ = child.Offset.X, child.Offset.Y, child.Offset.Z end)
								cp('{"ClassName":"SpecialMesh","MeshType":"'); cp(meshType); cp('"')
								if meshId ~= "" then cp(',"MeshId":"'); cp(meshId); cp('"') end
								if texId ~= "" then cp(',"TextureId":"'); cp(texId); cp('"') end
								cp(',"Scale":['); cp(string.format("%.4f,%.4f,%.4f", scX, scY, scZ))
								cp('],"Offset":['); cp(string.format("%.4f,%.4f,%.4f", ofX, ofY, ofZ)); cp(']}')
								childCount = childCount + 1; childJson[childCount] = table.concat(cbuf)
							end
						elseif child:IsA("BlockMesh") then
							...
							pcall(function() scX, scY, scZ = child.Scale.X, child.Scale.Y, child.Scale.Z end)
							pcall(function() ofX, ofY, ofZ = child.Offset.X, child.Offset.Y, child.Offset.Z end)
							cp('{"ClassName":"BlockMesh","Scale":[')
							cp(string.format("%.4f,%.4f,%.4f", scX, scY, scZ))
							cp('],"Offset":[')
							cp(string.format("%.4f,%.4f,%.4f", ofX, ofY, ofZ))
							cp(']}')
							childCount = childCount + 1; childJson[childCount] = table.concat(cbuf)
						elseif child:IsA("CylinderMesh") then
							...
							pcall(function() scX, scY, scZ = child.Scale.X, child.Scale.Y, child.Scale.Z end)
							pcall(function() ofX, ofY, ofZ = child.Offset.X, child.Offset.Y, child.Offset.Z end)
							cp('{"ClassName":"CylinderMesh","Scale":[')
							cp(string.format("%.4f,%.4f,%.4f", scX, scY, scZ))
							cp('],"Offset":[')
							cp(string.format("%.4f,%.4f,%.4f", ofX, ofY, ofZ))
							cp(']}')
				...
				if includeDecorations then
					for _, child in ipairs(part:GetChildren()) do
						if child:IsA("Smoke") or child:IsA("Fire") or child:IsA("Sparkles") then
							local cbuf = {}; local ci = 0
							local function cp(s) ci = ci + 1; cbuf[ci] = s end
							cp('{"ClassName":"'); cp(child.ClassName); cp('"')
							local okC, c = pcall(function() return child.Color end)
							if okC then cp(',"Color":['); cp(string.format("%d,%d,%d", math.floor(c.R*255+0.5), math.floor(c.G*255+0.5), math.floor(c.B*255+0.5))); cp(']') end
							local okS, s = pcall(function() return child.Size end)
							if okS then cp(',"Size":'); cp(string.format("%.4f", s)) end
							local okO, o = pcall(function() return child.Opacity end)
							if okO then cp(',"Opacity":'); cp(string.format("%.4f", o)) end
							local okR, r = pcall(function() return child.RiseVelocity end)
							if okR then cp(',"RiseVelocity":'); cp(string.format("%.4f", r)) end
							local okH, h = pcall(function() return child.Heat end)
							if okH then cp(',"Heat":'); cp(string.format("%.4f", h)) end
							local okSC, sc = pcall(function() return child.SecondaryColor end)
							if okSC then cp(',"SecondaryColor":['); cp(string.format("%d,%d,%d", math.floor(sc.R*255+0.5), math.floor(sc.G*255+0.5), math.floor(sc.B*255+0.5))); cp(']') end
							local okB, b = pcall(function() return child.Brightness end)
							if okB then cp(',"Brightness":'); cp(string.format("%.4f", b)) end
							local okSP, sp = pcall(function() return child.SparkleColor end)
							if okSP then cp(',"SparkleColor":['); cp(string.format("%d,%d,%d", math.floor(sp.R*255+0.5), math.floor(sp.G*255+0.5), math.floor(sp.B*255+0.5))); cp(']') end
							cp('}')
							childCount = childCount + 1; childJson[childCount] = table.concat(cbuf)
						end
					end
	...
	local function buildLua(filename, startIdx, endIdx, totalParts, canStream, chunkNum, totalChunks)
		...
		for i = startIdx, endIdx do
			...
			if includeDecals then
				for _, child in ipairs(part:GetChildren()) do
					if child:IsA("Decal") or child:IsA("Texture") then
						...
						local cprops = {"Texture", "Transparency", "Face", "Color3", "StudsPerTileU", "StudsPerTileV"}
			...
			if includeDecorations then
				for _, child in ipairs(part:GetChildren()) do
					if child:IsA("Smoke") or child:IsA("Fire") or child:IsA("Sparkles") then
						local cv = var .. "_dc" .. tostring(math.random(1000, 9999))
						push(string.format("local %s = Instance.new(%q)", cv, child.ClassName))
						local cprops = {"Color", "Size", "Opacity", "RiseVelocity", "Heat", "SecondaryColor", "Brightness", "SparkleColor"}
						for _, p in ipairs(cprops) do
							local ok, v = pcall(function() return child[p] end)
							if ok then push(string.format("%s.%s = %s", cv, p, fmtVal(v))) end
						end

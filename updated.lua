...
local function isValidAssetUrl(url)
	...
	if url:sub(1, 8) == "rbxasset" then return true end
	if url:sub(1, 7) == "http://" then return true end
...
local function applyPartProperties(part, partData, cf, partMap)
	...
	if className == "MeshPart" and (partData.MeshId or partData.TextureID or partData.RenderFidelity) then
		pcall(function()
			F3XRetry("CreateMeshes", {part})
			local mc = {Part = part}
			if partData.MeshId then mc.MeshId = partData.MeshId end
			if partData.TextureID then mc.TextureId = partData.TextureID end
		...
		for _, cd in ipairs(partData.Children) do
			pcall(function()
				if cd.ClassName == "Decal" or cd.ClassName == "Texture" then
					...
					local tc = {Part = part, Face = parseNormalId(cd.Face), TextureType = cd.ClassName}
					if cd.Texture then tc.Texture = cd.Texture end
				...
				elseif cd.ClassName == "SpecialMesh" then
					...
					if cd.MeshId then mc.MeshId = cd.MeshId end
					if cd.TextureId then mc.TextureId = cd.TextureId end
				...
				elseif cd.ClassName == "BlockMesh" or cd.ClassName == "CylinderMesh" then
					F3XRetry("CreateMeshes", {part})
					local mc = {Part = part}
					if cd.ClassName == "BlockMesh" then mc.MeshType = Enum.MeshType.Brick end
					if cd.ClassName == "CylinderMesh" then mc.MeshType = Enum.MeshType.Cylinder end
					if cd.Scale then mc.Scale = parseVector3(cd.Scale) end
					if cd.Offset then mc.Offset = parseVector3(cd.Offset) end
					F3XRetry("SyncMesh", {mc})
...
local function normalBuild(parts, total, partMap)
	...
	for i, partData in ipairs(parts) do
		...
		local part; local createOk = pcall(function() part = F3XRetry("CreatePart", partType, CFrame.new(0, 5000, 0)) end)
...
local function resetSelection()
	for _, instance in ipairs(selectionPool) do
		if instance and instance.Parent then instance:Destroy() end
	end
	selectionPool = {}; selectedParts = {}
end
...
local function getSelectionBounds(parts)
	...
	local center = (minPos + maxPos) / 2; local size = maxPos - minPos + Vector3.new(1, 1, 1)
	return CFrame.new(center), size
end

local function exportBuildToJSON(buildParts)
	local data = {Name = selectedBuildName or "Exported Build", PartCount = #buildParts, Parts = {}}
	for _, part in ipairs(buildParts) do
		if part and part:IsA("BasePart") then
			local pd = {ClassName = part.ClassName, Size = fmtV3(part.Size), Color = fmtC3(part.Color), CFrame = fmtCF(part.CFrame), Material = tostring(part.Material), Anchored = part.Anchored, CanCollide = part.CanCollide, Transparency = part.Transparency, Reflectance = part.Reflectance, CastShadow = part.CastShadow, Name = part.Name, Locked = part.Locked}
			data.Parts[#data.Parts + 1] = pd
		end
	end
	local ok, json = pcall(function() return HttpService:JSONEncode(data) end)
	if not ok then notify("JSON encode failed!"); return end
	local path = CONFIG.LocalFolder .. "/" .. (selectedBuildName or "export") .. ".json"
	if not isfolder(CONFIG.LocalFolder) then makefolder(CONFIG.LocalFolder) end
	local writeOk = writefile(path, json)
	if writeOk then notify("Exported " .. #buildParts .. " parts to " .. path)
	else notify("Write failed!") end
end

local function exportBuildToLua(buildParts)
	local lines = {}; lines[#lines + 1] = "-- F3X Build Export (Lua)"
	lines[#lines + 1] = "local buildData = {"
	lines[#lines + 1] = string.format("  Name = %q,", selectedBuildName or "Export")
	lines[#lines + 1] = string.format("  PartCount = %d,", #buildParts)
	lines[#lines + 1] = "  Parts = {"
	local buffer = {}; local bufCount = 0
	for _, part in ipairs(buildParts) do
		if part and part:IsA("BasePart") then
			...
			buffer[#buffer + 1] = pd; bufCount = bufCount + 1
			if bufCount >= CONFIG.StreamBatch then
				lines[#lines + 1] = table.concat(buffer, ",\\n"); buffer = {}; bufCount = 0
			end
		end
	end
	if bufCount > 0 then lines[#lines + 1] = table.concat(buffer, ",\\n") end
	lines[#lines + 1] = "  }"
	lines[#lines + 1] = "}"
	local path = CONFIG.LocalFolder .. "/" .. (selectedBuildName or "export") .. ".lua"
	if not isfolder(CONFIG.LocalFolder) then makefolder(CONFIG.LocalFolder) end
	local ok, err = pcall(function() writefile(path, table.concat(lines, "\\n")) end)
	if ok then notify("Exported as Lua to " .. path)
	else notify("Lua export failed: " .. tostring(err)) end
end

local function dragMarquee()
	local startPos = UserInputService:GetMouseLocation()
	local marquee = Instance.new("Frame"); marquee.Size = UDim2.new(0, 0, 0, 0)
	marquee.BackgroundColor3 = Color3.fromRGB(80, 180, 255); marquee.BackgroundTransparency = 0.7
	marquee.BorderSizePixel = 0; marquee.Parent = screenGui
	local marqueeActive = true; local conn = nil
	conn = RunService.Heartbeat:Connect(function()
		if not marqueeActive then if conn then conn:Disconnect() end return end
		local curPos = UserInputService:GetMouseLocation()
		local minX = math.min(startPos.X, curPos.X); local maxX = math.max(startPos.X, curPos.X)
		local minY = math.min(startPos.Y, curPos.Y); local maxY = math.max(startPos.Y, curPos.Y)
		marquee.Position = UDim2.new(0, minX, 0, minY)
		marquee.Size = UDim2.new(0, maxX - minX, 0, maxY - minY)
	end)
	local release; release = UserInputService.InputEnded:Connect(function(input, _)
		if not marqueeActive then return end
		if input.UserInputType == Enum.UserInputType.MouseButton1 then
			marqueeActive = false; if conn then conn:Disconnect() end; release:Disconnect()
			local endPos = UserInputService:GetMouseLocation()
			local minX = math.min(startPos.X, endPos.X); local maxX = math.max(startPos.X, endPos.X)
			local minY = math.min(startPos.Y, endPos.Y); local maxY = math.max(startPos.Y, endPos.Y)
			local width = maxX - minX; local height = maxY - minY
			if width < CONFIG.DragThreshold or height < CONFIG.DragThreshold then
				marquee:Destroy(); return
			end
			if UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) or UserInputService:IsKeyDown(Enum.KeyCode.RightControl) then
			else resetSelection() end
			local camPart = workspace.CurrentCamera
			local viewportSize = camPart.ViewportSize
			local camCF = camPart.CFrame; local fov = camPart.FieldOfView
			local tan = math.tan(math.rad(fov) / 2)
			local far = 10000; local near = 0.5
			local parts = workspace:GetDescendants()
			for _, p in ipairs(parts) do
				if p:IsA("BasePart") and p.Anchored and p ~= camPart then
					local screenPos, onScreen = camPart:WorldToViewportPoint(p.Position)
					if onScreen then
						local sx, sy = screenPos.X, screenPos.Y
						if sx >= minX and sx <= maxX and sy >= minY and sy <= maxY then
							local exists = false
							for _, sp in ipairs(selectedParts) do if sp == p then exists = true; break end end
							if not exists then
								selectedParts[#selectedParts + 1] = p
								local box = Instance.new("SelectionBox"); box.Color3 = Color3.fromRGB(80, 180, 255); box.LineThickness = 0.03; box.Adornee = p; box.Parent = p; selectionPool[#selectionPool + 1] = box
							end
						end
					end
				end
			end
			notify(#selectedParts .. " part(s) selected via marquee")
			marquee:Destroy()
		end
	end)
end
...
-- ==================== 16. TOOL BUTTON ACTIONS ====================
local function performToolAction(action)
	if not F3X:ValidateTool() then notify("F3X tools not ready!"); return end
	local function act(a)
		if a == "undo" then F3XRetry("Undo")
		elseif a == "redo" then F3XRetry("Redo")
		elseif a == "select" then dragMarquee()
		elseif a == "clear" then resetSelection()
		elseif a == "export-json" then if #selectedParts == 0 then notify("No parts selected!"); return end; exportBuildToJSON(selectedParts)
		elseif a == "export-lua" then if #selectedParts == 0 then notify("No parts selected!"); return end; exportBuildToLua(selectedParts)
		elseif a == "export-all" then
			local allParts = {}
			for _, p in ipairs(workspace:GetDescendants()) do
				if p:IsA("BasePart") and p.Anchored then allParts[#allParts + 1] = p end
			end
			if #allParts == 0 then notify("No anchored parts found!"); return end
			exportBuildToJSON(allParts)
		elseif a == "delete" then
			local count = 0
			for _, p in ipairs(selectedParts) do
				if p and p.Parent then count = count + 1; pcall(function() F3XRetry("RemovePart", p) end) end
			end
			notify("Deleted " .. count .. " part(s)"); resetSelection()
		elseif a == "clone" then
			local clones = {}
			for _, p in ipairs(selectedParts) do
				if p and p:IsA("BasePart") then
					pcall(function()
						local clone = F3XRetry("CreatePart", "Normal", CFrame.new(0, 5000, 0))
						clone.Size = p.Size; clone.Color = p.Color; clone.Material = p.Material; clone.CFrame = p.CFrame + Vector3.new(0, 0, -5)
						clones[#clones + 1] = clone
					end)
				end
			end
			resetSelection(); notify("Cloned " .. #clones .. " part(s)")
		elseif a == "group" then
			local boundsCF, boundsSize = getSelectionBounds(selectedParts)
			local group = Instance.new("Model"); group.Name = "Group"
			for _, p in ipairs(selectedParts) do p.Parent = group end
			group.Parent = workspace; resetSelection(); notify("Grouped " .. #selectedParts .. " parts")
		elseif a == "ungroup" then
			local count = 0
			for _, p in ipairs(selectedParts) do
				if p:IsA("Model") then
					for _, child in ipairs(p:GetChildren()) do
						if child:IsA("BasePart") then child.Parent = workspace; count = count + 1 end
					end
					p:Destroy()
				end
			end
			resetSelection(); notify("Ungrouped " .. count .. " parts")
		elseif a == "selectall" then
			resetSelection()
			for _, p in ipairs(workspace:GetDescendants()) do
			...
			notify("Selected " .. #selectedParts .. " anchored parts")
		end
	end
	if isBuilding then
		local ok = pcall(act, action)
		if not ok then notify("Tool action failed") end
	end
end

-- ==================== SELECTOR TAB BUTTONS ====================
do
	local yPos = 20; local function makeSelBtn(text, color, action)
		local btn = Instance.new("TextButton"); btn.Size = UDim2.new(1, 0, 0, 32)
		btn.Position = UDim2.new(0, 0, 0, yPos); btn.BackgroundColor3 = color
		btn.Text = text; btn.TextColor3 = THEME.TextPrimary; btn.TextSize = 11; btn.Font = Enum.Font.GothamBold
		btn.AutoButtonColor = true
		Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 6); btn.Parent = selectorTab
		btn.MouseButton1Click:Connect(function() performToolAction(action) end)
		yPos = yPos + 36; return btn
	end
	makeSelBtn("??? Marquee Select", Color3.fromRGB(0, 120, 200), "select")
	makeSelBtn("??? Delete Selected", THEME.Danger, "delete")
	makeSelBtn("?? Clone Selected", Color3.fromRGB(100, 160, 80), "clone")
	makeSelBtn("?? Group Selected", Color3.fromRGB(140, 100, 200), "group")
	makeSelBtn("?? Ungroup Selected", Color3.fromRGB(200, 130, 100), "ungroup")
	makeSelBtn("?? Select All Anchored", Color3.fromRGB(0, 100, 150), "selectall")
	makeSelBtn("?? Export Selected (JSON)", Color3.fromRGB(60, 160, 80), "export-json")
	makeSelBtn("?? Export Selected (Lua)", Color3.fromRGB(60, 130, 180), "export-lua")
	makeSelBtn("?? Export All Anchored", Color3.fromRGB(80, 120, 200), "export-all")
	local yRest = yPos
	local buildSelBtn = Instance.new("TextButton")
	buildSelBtn.Size = UDim2.new(1, 0, 0, 32); buildSelBtn.BackgroundColor3 = THEME.Success
	buildSelBtn.Text = "?? Build Button"; buildSelBtn.Visible = false
	buildSelBtn.TextColor3 = THEME.TextPrimary; buildSelBtn.TextSize = 12; buildSelBtn.Font = Enum.Font.GothamBold
	buildSelBtn.AutoButtonColor = true
	Instance.new("UICorner", buildSelBtn).CornerRadius = UDim.new(0, 6); buildSelBtn.Parent = selectorTab
	local buildBtnSel; buildBtnSel = RunService.Heartbeat:Connect(function()
		if buildBtn.Visible then
			buildSelBtn.Position = UDim2.new(0, 0, 0, yRest)
			buildSelBtn.Visible = true; buildSelBtn.Parent = selectorTab
			buildBtnSel:Disconnect()
		end
	end)
	goToBuildBtn = Instance.new("TextButton")
	goToBuildBtn.Size = UDim2.new(1, 0, 0, 32); goToBuildBtn.BackgroundColor3 = Color3.fromRGB(60, 140, 200)
	goToBuildBtn.Text = "?? Teleport to Build"; goToBuildBtn.Visible = false
	goToBuildBtn.TextColor3 = THEME.TextPrimary; goToBuildBtn.TextSize = 12; goToBuildBtn.Font = Enum.Font.GothamBold
	goToBuildBtn.AutoButtonColor = true
	Instance.new("UICorner", goToBuildBtn).CornerRadius = UDim.new(0, 6); goToBuildBtn.Parent = selectorTab
end

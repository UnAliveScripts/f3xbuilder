--[[
 ============================================
 F3X BUILD LOADER (ULTIMATE EDITION)
 - Anchor Toggle: Build as Original / Build Fully Anchored
 - Exact replication with SyncMove for precise CFrame
 - HYPER BUILD MODE: Multi-threaded parallel building
 - ALL F3X Building Tool features supported
 - Unlimited sliders + scrolling
...
-- Decorations (Smoke, Fire, Sparkles)
...
local function parseLightType(str)
 ...
 elseif name:find("Spot") then return "SpotLight"
 end
 return "PointLight"
end
...
local function parseDecoType(str)
 ...
 elseif name:find("Sparkles") then return "Sparkles"
 end
 return "Smoke"
end
...
localFrame.Size = UDim2.new(1, -20, 0, 160)
...
githubFrame.Size = UDim2.new(1, -20, 0, 160)
...
toggleFrame.Position = UDim2.new(0, 10, 0, 258)
...
anchorToggleBtn.MouseButton1Click:Connect(function()
 ...
 anchorToggleBtn.Text = "⚓ Fully Anchored"
 anchorToggleBtn.BackgroundColor3 = Color3.fromRGB(0, 130, 80)
 else
 anchorToggleBtn.Text = "⚓ As Original"
 anchorToggleBtn.BackgroundColor3 = Color3.fromRGB(60, 40, 80)
 end
...
hyperToggleBtn.TextSize = 12
hyperToggleBtn.Font = Enum.Font.GothamBold
hyperToggleBtn.AutoButtonColor = true
Instance.new("UICorner", hyperToggleBtn).CornerRadius = UDim.new(0, 6)
hyperToggleBtn.Parent = toggleFrame

hyperToggleBtn.MouseButton1Click:Connect(function()
 CONFIG.HyperMode = not CONFIG.HyperMode
 if CONFIG.HyperMode then
 hyperToggleBtn.Text = "🚀 HYPER MODE"
 hyperToggleBtn.BackgroundColor3 = Color3.fromRGB(200, 50, 50)
 notify("HYPER MODE enabled! Multi-threaded building active!")
 else
 hyperToggleBtn.Text = "🚀 Normal Mode"
 hyperToggleBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 50)
...
slidersFrame.Size = UDim2.new(1, -20, 0, 150)
slidersFrame.Position = UDim2.new(0, 10, 0, 300)
...
-- Threads Slider Row (for Hyper Mode)
...
threadsLabel.TextColor3 = Color3.fromRGB(200, 200, 210)
threadsLabel.TextSize = 11
threadsLabel.Font = Enum.Font.Gotham
threadsLabel.TextXAlignment = Enum.TextXAlignment.Left
threadsLabel.Parent = slidersFrame
...
threadsValueBox.BackgroundColor3 = Color3.fromRGB(35, 35, 45)
threadsValueBox.Text = "4"
...
threadsValueBox.TextSize = 12
threadsValueBox.Font = Enum.Font.GothamBold
threadsValueBox.ClearTextOnFocus = false
Instance.new("UICorner", threadsValueBox).CornerRadius = UDim.new(0, 4)
threadsValueBox.Parent = slidersFrame
...
threadsUnitLabel.Text = "parallel"
threadsUnitLabel.TextColor3 = Color3.fromRGB(150, 150, 160)
threadsUnitLabel.TextSize = 10
threadsUnitLabel.Font = Enum.Font.Gotham
threadsUnitLabel.TextXAlignment = Enum.TextXAlignment.Left
threadsUnitLabel.Parent = slidersFrame
...
threadsSliderBg.BorderSizePixel = 0
threadsSliderBg.Parent = slidersFrame
...
threadsSliderFill.BorderSizePixel = 0
threadsSliderFill.Parent = threadsSliderBg
...
threadsSliderKnob.Text = ""
threadsSliderKnob.Parent = threadsSliderBg
...
-- Threads value logic
local threadsValue = 4
local function updateThreadsFromValue(val)
 val = math.max(1, math.min(32, math.floor(val)))
 threadsValue = val
 CONFIG.HyperThreads = val
 threadsValueBox.Text = tostring(val)
...
threadsValueBox.FocusLost:Connect(function()
 ...
 updateThreadsFromValue(threadsValue)
 end
end)
...
local function applyPartProperties(part, partData, cf, partMap)
 ...
 -- ANCHOR (respect toggle setting)
 local shouldAnchor = CONFIG.BuildAnchored or (partData.Anchored == true)
 table.insert(anchorChanges, {Part = part, Anchored = shouldAnchor})
 ...
 table.insert(rotateChanges, {Part = part, CFrame = cf * CFrame.Angles(math.rad(ori.X), math.rad(ori.Y), math.rad(ori.Z))})
 end

 -- Apply all property changes via F3X
 ...
 pcall(function() if #rotateChanges > 0 then F3X:SyncRotate(rotateChanges) end end)
 ...
 -- CHILDREN (Decals, Textures, SpecialMeshes, Lights, Decorations, Welds)
 ...
 -- DECORATIONS
 elseif childData.ClassName == "Smoke" or childData.ClassName == "Fire" or childData.ClassName == "Sparkles" then
 F3X:CreateDecorations({{Part = part, DecorationType = childData.ClassName}})
 local decoChanges = {Part = part, DecorationType = childData.ClassName}
 if childData.Color then decoChanges.Color = parseColor3(childData.Color) end
 if childData.Size ~= nil then decoChanges.Size = childData.Size end
 if childData.Opacity ~= nil then decoChanges.Opacity = childData.Opacity end
 if childData.RiseVelocity ~= nil then decoChanges.RiseVelocity = childData.RiseVelocity end
 if childData.Brightness ~= nil then decoChanges.Brightness = childData.Brightness end
 if childData.Heat ~= nil then decoChanges.Heat = childData.Heat end
 if childData.SecondaryColor then decoChanges.SecondaryColor = parseColor3(childData.SecondaryColor) end
 if childData.SparkleColor then decoChanges.SparkleColor = parseColor3(childData.SparkleColor) end
 F3X:SyncDecorate({decoChanges})

 -- WELD (child weld)
 elseif childData.ClassName == "Weld" or childData.ClassName == "WeldConstraint" then
 if childData.Part0Index and childData.Part1Index and partMap then
 local p0 = partMap[childData.Part0Index]
 local p1 = partMap[childData.Part1Index]
 if p0 and p1 then
...
local function normalBuild(parts, total, partMap)
 ...
 -- EXACT REPLICATION: Create at origin first, then SyncMove to exact CFrame
 local part
 local createOk, createResult = pcall(function()
 part = F3X:CreatePart(partType, CFrame.new(0, 5000, 0))
 return part
 end)
 ...
 -- Move to exact CFrame immediately for precision
 local moveOk = pcall(function()
 F3X:SyncMove({{Part = part, CFrame = cf}})
 end)
 if not moveOk then
...
-- ==================== HYPER BUILD FUNCTION ====================
local function hyperBuild(parts, total, partMap)
 local threadCount = CONFIG.HyperThreads
 local batchSize = CONFIG.BatchSize
 local buildSpeed = CONFIG.BuildSpeed
 local failedCount = 0
 ...
 for i, partData in ipairs(parts) do
 local chunkIndex = ((i - 1) % threadCount) + 1
 table.insert(chunks[chunkIndex], {Index = i, Data = partData})
 end
 ...
 -- Update progress (thread-safe-ish via simple lock)
 if completedCount % batchSize == 0 or completedCount == total then
 ...
 for t = 1, threadCount do
 while coroutine.status(threads[t]) ~= "dead" do
 task.wait(0.1)
 end
 end
...
buildBtn.MouseButton1Click:Connect(function()
 ...
 -- Build all parts (Normal or Hyper mode)
 if CONFIG.HyperMode then
 ...
 local anchorMsg = CONFIG.BuildAnchored and " [ALL ANCHORED]" or " [AS ORIGINAL]"
 local modeMsg = CONFIG.HyperMode and " [HYPER]" or ""
 local msg = string.format("✅ Built %d/%d parts!%s%s", total - failedCount, total, anchorMsg, modeMsg)
